...
public class Network {
    ...
    /*
     * Apply a set of trained weights to the entire network 
     *  (assuming there is only one hidden layer) 
     */
    public void updateNetworkWeights(float[][] w_ILayer, float[][] w_HLayer) {
        for (int i = 0; i < this.getINodes().length; i++) {
            this.getINodes()[i].setWeight(w_ILayer[i]);
        }
        for (int i = 0; i < this.getL1HNodes().length; i++) {
            this.getL1HNodes()[i].setWeight(w_HLayer[i]);
        }
    }
    ...
    private void setupInputNodes() {
        ...
        if (HLayers == 0) {
         ...
         nodes in the first hidden layer */ else if (HLayers > 0) {
            for (int i = 0; i < inputN.length; i++) {
                inputN[i] = new INode(inputVector[i], weight);
            ...
            if (i == 0) {
                for (int j = 0; j < layer1.length; j++) {
                    ...
                    if (HLayers == 1) {
                     ...
                     of each node in the first hidden layer is equal to the
                     number of nodes in the second hidden layer*/ else if (HLayers == 2) {
                        layer1[j] = new HNode(weight);
    ...
    /**
     * Shane wrote original, James adjusted input parameter An activation
    ...
    /**
     * Shane wrote original, James adjusted input parameter An activation
     * function using a hyperbolic tangent.
     */
